...
for i in range(len(squares)): 
    ...
    theta1 = math.radians((90-angle2)/2)
    theta2 = math.radians((angle2/2))
    extra = int(1.414*padded.shape[0]*math.sin(theta1)*math.sin(theta2))

    x_offset = int(sq.getCentreSquare(squares[i])[0] - sizeOfSquares[i]/2 - extra)
    y_offset = int(sq.getCentreSquare(squares[i])[1] - sizeOfSquares[i]/2 - extra)
